...
def pit_page():
    ...
    matches = sorted(matches, key=lambda match: match["predicted_time"])
...
def alliance_page():
    ...
    redEPA = sum([sb.get_team_year(int(team), int(config["YEAR"]), ["epa_end"])["epa_end"] for team in red_alliance])
    blueEPA = sum([sb.get_team_year(int(team), int(config["YEAR"]), ["epa_end"])["epa_end"] for team in blue_alliance])

    red_score = round(redEPA * 1.065)
    blue_score = round(blueEPA * 1.065)

    percentage = round(100 / (1 + 10 ** ((-5/8 * (redEPA - blueEPA)) / sb.get_year(int(config["YEAR"]), ["score_sd"])["score_sd"])), 2)
